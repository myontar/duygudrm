...
def connect(request):
    ...
    try:
        ...
        wrap.app_id = settings.LIVE_APP_ID
        wrap.app_secret = settings.LIVE_APP_SECRET
        wrap.callback = "http://framemind.com/messenger/connected"

        wrap.scope = settings.LIVE_DEFAULT_SCOPE

        url = wrap.build_consent_url()

        request.session['wrap2'] = wrap
        response = HttpResponseRedirect(url)
    except KeyError:
        response = __redirect_on_bad_request()
...
def connected(request):
    ...
    try:
        ...
        wrap.consent_response = request.GET
        request.session['wrap2'] = wrap
        response = HttpResponseRedirect('/messenger/accessconnect')
    except KeyError:
        response = __redirect_on_bad_request()
...
def access_request(request):
    ...
    try:
        ...
        response = HttpResponseRedirect('/msn_connect')
    except KeyError:
        response = __redirect_on_bad_request()

    return response
def accessconnect(request):
    #Requests authorization tokens via the 'access' URL.
    try:
        wrap = request.session['wrap2']
        #wrap.app_secret = wrap.consent_response['accessAppSecret']
        ...
        request.session['wrap2'] = wrap
        request.session['userinfo'] = wrap.userdata()
        response = HttpResponseRedirect('/msnconnect')
    except KeyError:
        response = __redirect_on_bad_request()
