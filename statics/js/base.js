...
<<<<<<< HEAD
		...
		editComment:function() {
			...
			html.find("button").click(function(){
				...
				k = this
				main.sendPost("/","ec="+cid+"&id="+id+"&text="+ret_text,function() {main.update();jQuery(k).parent().slideUp(1000,function() {jQuery(k).parent().remove();par.text(ret_text)})});
			});
...
<<<<<<< HEAD
			...
			if (ajax_pos == 1) {
				main.sendPost("/",elm,function(data){
				  ...
				  if (parseInt(r['alerts']) > 0 ) {
					jQuery(".walerts").html(r['alerts']);
					if(jQuery(".walerts").css("display") == "none") {
						jQuery(".walerts").show("slow");
					} else {
						if(alert_count < parseInt(r['alerts']) )jQuery(".walerts").effect("highlight", {}, 3000);
					}
					alert_count = parseInt(r['alerts']);
				  }
